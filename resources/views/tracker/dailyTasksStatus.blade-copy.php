			...
			<div class="row">
				<div class="bs-example">
				    <div class="panel-group" id="accordion">
				        <div class="panel panel-default"  ng-repeat = "task in form.taskdetails track by $index">
				            <div class="panel-heading">
				                <h4 class="panel-title">
				                    <a data-toggle="collapse" data-parent="#accordion" href="#collapse@{{ $index }}"><b>@{{ task.date }}</b></a>
				                </h4>
				            </div>
				            <div id="collapse@{{ $index }}" ng-class="{'in':$index == 0}"  class="panel-collapse collapse">
				                <div class="panel-body">
				                	<div class="table-responsive">
				                        ...
				                        <tbody>
				                            <tr ng-repeat="(key, ta) in task.result">
				                                ...
				                                <td ng-repeat="(optkey, optval) in form.option">
				                                	<input type="text" name="" ng-model="task.result[key][optkey]" value="@{{ task.result[key][optkey] }}" ng-disabled="form.loginstudentId != key">
				                                </td>
				                            </tr>
				                        </tbody>
			                    		<table>
				...
				<br>
				<div class="well">
					<button class="btn btn-success" ng-if="form.taskdetails.length > 0"  ng-click="saveData()">Save</button>
				                ...
				                <div class="panel-body">
				                	<div class="table-responsive">
				                        ...
				                        <thead>
				                            <tr>
				                                <th>Students</th>
				                                <th ng-repeat="opt in form.optionscolour">@{{ opt }} @{{ task.result  }} </th>
				                            </tr>
				                        </thead>
				                        <tbody>
				                            <tr ng-repeat="(key, ta) in task.result track by $index">
				                                <td ng-show="task.result[key].alternative != ''">@{{ task.result[key].name }}</td>
				                                <td ng-show="task.result[key].alternative == ''">@{{ task.result[key].alternative }}</td>
				                                <td ng-repeat="(optkey, optval) in form.optionscolour" style="color:@{{ task.result[key].colours[optkey] }}">
				                                	@{{ task.result[key][optkey] }} - @{{ task.result[key]}}
													@{{ task.result  }}
				                                </td>	
				                            </tr>
				                        </tbody>
	...
	<script type="text/javascript">
	    ...
	    app.controller('ezTrackStdCntrl', function($scope,$http,$timeout) {
	    	...
	    	$scope.searchData = function(){
	    		...
	    		if($scope.form.startDate != null && $scope.form.taskname != null && $scope.form.selecttask != null){
		            var data = $scope.form;
		            console.log(data);
